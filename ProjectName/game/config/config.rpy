...
init python:
	...
	def replace_text(s):
		import re
		# Add a pause after dashes *not* used to indicate text being cut off
		s = re.sub("--(?!$)", "--{w=.15}", s)
		# Turn those double dashes into em-dashes.
		s = s.replace("--", u"\u2014")
		# Add a pause after commas. Those are pretty safe.
		s = s.replace(__(",{#Comma Substring}"), ",{w=.15}")
		s = s.replace(__(";{#Semi-colon Substring}"), ";{w=.15}")
		s = re.sub(__("(?:[!?])+(?:[\\)\"\'])?\\s{#Non-Period Terminal Regex}"), "\\g<0>{w=.3}", s)
		s = re.sub(
		__('(?<!((\\b)(Ave|etc|lbs|Mrs)))(?<!((\\b)(Dr|Mr|Ms|Rd|St|vs|\\..)))\\.\\)?[\"\']?\\s{#Period Regex}'), "\\g<0>{w=.3}", s)
		return s
